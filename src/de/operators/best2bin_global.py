...
def best2bin_global(self, target):
    if not self.de.sade_run:
        sade_k = 0
    else:
        sade_k = self.de.sade_ops.index(self.best2bin_global)
    self.de.sade_k = sade_k

    cutPoint = random.randint(0, self.de.rosetta_pack.pose.total_residue())
    t_angle = []
    ind1 = self.get_best_individual()
    ind2, ind3, ind4, ind5 = self.get_random_individuals(n=4)
    index, c, d = 0, 0, 0
